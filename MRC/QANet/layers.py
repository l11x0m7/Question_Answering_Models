...
def conv_block(inputs, num_conv_layers, kernel_size, num_filters,
    ...
    """
    inputs: [batch, q_len, num_filters]

    outputs: [batch, q_len, num_filters], conv_num
    """
    with tf.variable_scope(scope, reuse = reuse):
...
def self_attention_block(inputs, num_filters, seq_len, mask = None, num_heads = 8,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        outputs = conv(outputs, num_filters, bias=True, activation=None, name = "FFN_2", reuse = reuse)
        outputs = layer_dropout(outputs, residual, dropout * float(l) / L)
        l += 1
        return outputs, l
...
def depthwise_separable_convolution(inputs, kernel_size, num_filters,
    ...
    """
    inputs: [batch, q_len, 1, dim]

    outputs: [batch, q_len, 1, num_filters]
    """
    with tf.variable_scope(scope, reuse = reuse):
...
def conv(inputs, output_size, bias = False, 
         activation = None, 
         kernel_size = 1, 
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if len(input_shape) not in (3, 4):
            ...
            filter_shape = [1, kernel_size, input_shape[-1], output_size]
            bias_shape = [1, 1, 1, output_size]
            stride = [1, 1, 1, 1]
        else:
            filter_shape = [kernel_size, input_shape[-1], output_size]
            bias_shape = [1, 1, output_size]
            stride = 1

        conv_func = tf.nn.conv2d if len(input_shape) == 4 else tf.nn.conv1d
        kernel_ = tf.get_variable(name='k', 
                                  shape=filter_shape, 
                                  dtype=np.float32, 
                                  regularizer=regularizer,
                                  initializer=initializer_relu()
                                    if activation is not None else initializer())
        outputs = conv_func(inputs, kernel_, stride, padding='VALID')
        if bias:
            outputs += tf.get_variable('bias', 
                                       shape=bias_shape, 
                                       dtype=np.float32,
                                       regularizer=regularizer,
                                       initializer=tf.zeros_initializer())
...
def multihead_attention(queries, units, num_heads,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        if memory is None:
            memory = queries
        memory = conv(memory, units * 2, 
                      name='memory_projection', reuse=reuse)
        query = conv(queries, units, 
                      name='query_projection', reuse=reuse)
        # [batch, heads, length_q, units/heads]
        Q = split_last_dimension(query, num_heads)
        # [batch, heads, length_k, units/heads]
        K, V = [split_last_dimension(m, num_heads) for m in tf.split(memory, 2, axis=-1)]
...
def split_last_dimension(x, n):
    ...
    n: an integer.
    Returns:
    a Tensor with shape [..., n, m/n] -> [batch_size, n, p_len, m/n]
    """
...
def combine_last_two_dimensions(x):
    ...
    ret = tf.reshape(x, tf.concat([tf.shape(x)[:-2], a * b], axis=0))
    ret.set_shape(new_shape)
    return ret
...
def dot_product_attention(q,
    ...
    mask: a Tensor with shape [batch, length_kv]
    Returns:
    A Tensor.
    """
